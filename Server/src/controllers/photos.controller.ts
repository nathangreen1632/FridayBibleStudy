...
import { deletePhotoIfAllowed, listPhotoDtos, resolveTargetPrayerId, saveUploadedPhotos, validateUploadBatch } from '../services/photos.service.js';
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const targetPrayerId = await resolveTargetPrayerId(Number.isFinite(explicit) ? explicit : undefined, req.user);
      if (!targetPrayerId) {
        res.status(400).json({ error: 'No target post found to attach photos. Create a post first or provide a prayerId.' });
        return;
      }

      const createdIds = await saveUploadedPhotos(files, targetPrayerId);
